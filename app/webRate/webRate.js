/**
 * Created by penn on 2015/9/23.
 */
angular.module('myApp.webRate', ['ngRoute','pdiService'])
    .config(['$routeProvider', function($routeProvider) {
        $routeProvider.when('/webRate', {
            templateUrl: 'webRate/webRate.html',
            controller: 'webRateCtrl'
        });
    }])
    .controller('webRateCtrl',['$scope','rateService',function($scope, service){
        $scope.symbol = "GOOG";
        $scope.items = [];
        $scope.startDate = '2014-12-05';
        $scope.endDate = '2015-12-06';
        $scope.options = [{
            value: '$q'},
            {
                value: 'callback'},
